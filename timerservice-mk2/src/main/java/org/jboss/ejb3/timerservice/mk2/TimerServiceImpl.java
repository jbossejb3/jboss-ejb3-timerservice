...
public class TimerServiceImpl implements TimerService {
	...
	/**
	 * Holds the {@link Future} of each of the timer tasks that have been
	 * scheduled
	 */
	 ...
	 * 
	 * @param invoker
	 *            The {@link TimedObjectInvoker} responsible for invoking the
	 *            timeout method
	 * @param emf
	 *            Entity manager factory responsible for JPA persistence
	 *            management
	 * @param transactionManager
	 *            Transaction manager responsible for managing the transactional
	 *            timer service
	 * @param executor
	 *            Executor service responsible for creating scheduled timer
	 *            tasks
	 * @throws IllegalArgumentException
	 *             If either of the passed param is null
	 */
	public TimerServiceImpl(TimedObjectInvoker invoker,
	...
	public Timer createCalendarTimer(ScheduleExpression schedule,
		...
		boolean persistent = timerConfig == null ? true : timerConfig
				.isPersistent();
	...
	public Timer createIntervalTimer(Date initialExpiration,
		...
		if (initialExpiration == null) {
			throw new IllegalArgumentException(
					"initialExpiration cannot be null while creating a timer");
		}
		if (initialExpiration.getTime() < 0) {
			throw new IllegalArgumentException(
					"initialExpiration.getTime() cannot be negative while creating a timer");
		}
		if (intervalDuration < 0) {
			throw new IllegalArgumentException(
					"intervalDuration cannot be negative while creating a timer");
		}
		return this.createTimer(initialExpiration, intervalDuration,
	...
	public Timer createIntervalTimer(long initialDuration,
		...
		if (initialDuration < 0) {
			throw new IllegalArgumentException(
					"initialDuration cannot be negative while creating interval timer");
		}
		if (intervalDuration < 0) {
			throw new IllegalArgumentException(
					"intervalDuration cannot be negative while creating interval timer");
		}

		return this.createIntervalTimer(new Date(System.currentTimeMillis()
	...
	public Timer createSingleActionTimer(Date expiration,
		...
		if (expiration == null) {
			throw new IllegalArgumentException(
					"expiration cannot be null while creating a single action timer");
		}
		if (expiration.getTime() < 0) {
			throw new IllegalArgumentException(
					"expiration.getTime() cannot be negative while creating a single action timer");
		}
		return this.createTimer(expiration, 0, timerConfig.getInfo(),
	...
	public Timer createSingleActionTimer(long duration, TimerConfig timerConfig)
		...
		if (duration < 0)
			throw new IllegalArgumentException(
					"duration cannot be negative while creating single action timer");

		return createTimer(new Date(System.currentTimeMillis() + duration), 0,
				timerConfig.getInfo(), timerConfig.isPersistent());
	}
	...
	public Timer createTimer(long duration, Serializable info)
		...
		if (duration < 0)
			throw new IllegalArgumentException(
					"Duration cannot negative while creating the timer");
	...
	public Timer createTimer(Date expiration, Serializable info)
		...
		if (expiration == null) {
			throw new IllegalArgumentException(
					"Expiration date cannot be null while creating a timer");
		}
		if (expiration.getTime() < 0) {
			throw new IllegalArgumentException(
					"expiration.getTime() cannot be negative while creating a timer");
		}
	...
	public Timer createTimer(long initialDuration, long intervalDuration,
		...
		if (initialDuration < 0) {
			throw new IllegalArgumentException(
					"Initial duration cannot be negative while creating timer");
		}
		if (intervalDuration < 0) {
			throw new IllegalArgumentException(
					"Interval cannot be negative while creating timer");
		}
		return this.createTimer(new Date(System.currentTimeMillis()
				+ initialDuration), intervalDuration, info, true);
	...
	public Timer createTimer(Date initialExpiration, long intervalDuration,
		...
		if (initialExpiration == null) {
			throw new IllegalArgumentException(
					"intial expiration date cannot be null while creating a timer");
		}
		if (initialExpiration.getTime() < 0) {
			throw new IllegalArgumentException(
					"expiration.getTime() cannot be negative while creating a timer");
		...
		if (intervalDuration < 0) {
			throw new IllegalArgumentException(
					"interval duration cannot be negative while creating timer");
		}
		return this
				.createTimer(initialExpiration, intervalDuration, info, true);
	}

	@Override
	public org.jboss.ejb3.timerservice.extension.Timer getAutoTimer(
			ScheduleExpression schedule, Method timeoutMethod) {
	...
	public org.jboss.ejb3.timerservice.extension.Timer getAutoTimer(
		...
		return this.createCalendarTimer(schedule, timerConfig.getInfo(),
				timerConfig.isPersistent(), timeoutMethod);
	}
	...
	public Collection<Timer> getTimers() throws IllegalStateException,
		...
		if (this.isLifecycleCallbackInvocation()
				&& this.isSingletonBeanInvocation() == false) {
		...
		for (TimerImpl timer : this.persistentWaitingOnTxCompletionTimers
				.values()) {
			if (timer != null && timer.isActive()) {
				activeTimers.add(timer);
			}
	 ...
	 *            The {@link Date} at which the first timeout should occur.
	 *            <p>
	 *            If the date is in the past, then the timeout is triggered
	 *            immediately when the timer moves to {@link TimerState#ACTIVE}
	 *            </p>
	 * @param intervalDuration
	 *            The interval (in milli seconds) between consecutive timeouts
	 *            for the newly created timer.
	 *            <p>
	 *            Cannot be a negative value. A value of 0 indicates a single
	 *            timeout action
	 *            </p>
	 * @param info
	 *            {@link Serializable} info that will be made available through
	 *            the newly created timer's {@link Timer#getInfo()} method
	 * @param persistent
	 *            True if the newly created timer has to be persistent
	 * @return Returns the newly created timer
	 * @throws IllegalArgumentException
	 *             If <code>initialExpiration</code> is null or
	 *             <code>intervalDuration</code> is negative
	 * @throws IllegalStateException
	 *             If this method was invoked during a lifecycle callback on the
	 *             EJB
	 */
	private Timer createTimer(Date initialExpiration, long intervalDuration,
			Serializable info, boolean persistent) {
		if (this.isLifecycleCallbackInvocation()
				&& this.isSingletonBeanInvocation() == false) {
			throw new IllegalStateException(
					"Creation of timers is not allowed during lifecycle callback of non-singleton EJBs");
		}
		if (initialExpiration == null) {
		...
		// create the timer
		TimerImpl timer = new TimerImpl(uuid.toString(), this,
				initialExpiration, intervalDuration, info, persistent);
		// if it's persistent, then save it
		if (persistent) {
			this.persistTimer(timer);
		}
		// now "start" the timer. This involves, moving the timer to an ACTIVE
		// state
	 ...
	 * 
	 * @param schedule
	 *            The {@link ScheduleExpression} which will be used for creating
	 ...
	 * @return Returns the newly created timer
	 * @throws IllegalArgumentException
	 *             If the passed <code>schedule</code> is null
	 * @throws IllegalStateException
	 *             If this method was invoked during a lifecycle callback on the
	 *             EJB
	 */
	private org.jboss.ejb3.timerservice.extension.Timer createCalendarTimer(
			ScheduleExpression schedule, Serializable info, boolean persistent,
			Method timeoutMethod) {
		if (this.isLifecycleCallbackInvocation()
				&& this.isSingletonBeanInvocation() == false) {
			throw new IllegalStateException(
		...
		CalendarBasedTimeout calendarTimeout = new CalendarBasedTimeout(
				schedule);

		// checking if there is another timer configured for the same method and
		// same scheduling...
		// this is a workaround for having a single database cluster wise and
		...
		if (timeoutMethod != null) {
			...
			for (TimerImpl timerImpl : activeTimers) {
				...
				if (timerImpl.isAutoTimer()) {
					// is the same method?
					CalendarTimer otherTimer = (CalendarTimer) timerImpl;
					Method otherTimerMethod = otherTimer.getTimeoutMethod();
		...
		UUID uuid = UUID.randomUUID();
		// create the timer
		TimerImpl timer = new CalendarTimer(uuid.toString(), this,
				calendarTimeout, info, persistent, timeoutMethod);
	...
	private boolean isEquals(ScheduleExpression thisExpr,
			ScheduleExpression otherExpr) {
		if (thisExpr.getDayOfMonth() == null) {
			if (otherExpr.getDayOfMonth() != null)
				return false;
		} else if (!thisExpr.getDayOfMonth().equals(otherExpr.getDayOfMonth()))
			return false;
		if (thisExpr.getDayOfWeek() == null) {
			if (otherExpr.getDayOfWeek() != null)
				return false;
		} else if (!thisExpr.getDayOfWeek().equals(otherExpr.getDayOfWeek()))
			return false;
		if (thisExpr.getEnd() == null) {
			if (otherExpr.getEnd() != null)
				return false;
		} else if (!thisExpr.getEnd().equals(otherExpr.getEnd()))
			return false;
		if (thisExpr.getHour() == null) {
			if (otherExpr.getHour() != null)
				return false;
		} else if (!thisExpr.getHour().equals(otherExpr.getHour()))
			return false;
		if (thisExpr.getMinute() == null) {
			if (otherExpr.getMinute() != null)
				return false;
		} else if (!thisExpr.getMinute().equals(otherExpr.getMinute()))
			return false;
		if (thisExpr.getMonth() == null) {
			if (otherExpr.getMonth() != null)
				return false;
		} else if (!thisExpr.getMonth().equals(otherExpr.getMonth()))
			return false;
		if (thisExpr.getSecond() == null) {
			if (otherExpr.getSecond() != null)
				return false;
		} else if (!thisExpr.getSecond().equals(otherExpr.getSecond()))
			return false;
		if (thisExpr.getStart() == null) {
			if (otherExpr.getStart() != null)
				return false;
		} else if (!thisExpr.getStart().equals(otherExpr.getStart()))
			return false;
		if (thisExpr.getTimezone() == null) {
			if (otherExpr.getTimezone() != null)
				return false;
		} else if (!thisExpr.getTimezone().equals(otherExpr.getTimezone()))
			return false;
		if (thisExpr.getYear() == null) {
			if (otherExpr.getYear() != null)
				return false;
		} else if (!thisExpr.getYear().equals(otherExpr.getYear()))
			return false;
		return true;
	}

	/**
	 * TODO: Rethink about this method. Do we really need this? Adds the timer
	 * instance to an internal {@link TimerHandle} to {@link TimerImpl} map.
	 * 
	 * @param timer
	 *            Timer instance
	 */
	protected void addTimer(TimerImpl timer) {
		if (timer.persistent == false) {
			...
			synchronized (this.persistentWaitingOnTxCompletionTimers) {
				this.persistentWaitingOnTxCompletionTimers.put(
						timer.getTimerHandle(), timer);
			}
		}
	}

	/**
	 * Returns the {@link ScheduledExecutorService} used for scheduling timer
	 * tasks
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the {@link TimedObjectInvoker} to which this timer service
	 * belongs
	 * 
	 * @return
	 */
	 ...
	 * 
	 * @param handle
	 *            The {@link TimerHandle} for which the {@link Timer} is being
	 *            looked for
	 * @return TODO: Rethink about this method. Looks brittle.
	 */
	public org.jboss.ejb3.timerservice.extension.Timer getTimer(
			TimerHandle handle) {
		// TODO: Why do we even have to maintain a map of timers.
		// Looks needless and could perhaps lead to inconsistencies.
		// Let's instead just rely on DB? But then again what about
	 ...
	 * @throws EJBException
	 *             If there is any system level exception
	 */
	...
	void removeTimer(TimerImpl timer) {
		if (timer.persistent == false) {
			...
			synchronized (this.persistentWaitingOnTxCompletionTimers) {
				this.persistentWaitingOnTxCompletionTimers.remove(timer
						.getTimerHandle());
			}
		}
	}
	 ...
	 * 
	 * <p>
	 * If the passed timer is null or is non-persistent (i.e.
	 * {@link Timer#isPersistent()} returns false), then this method acts as a
	 * no-op
	 * </p>
	 * 
	 * @param timer
	...
	public boolean refreshAndLockTimer(TimerImpl timer) {
		...
		// get the persistent entity from the timer
		TimerEntity timerEntity = timer.persistentState;

		if(timerEntity == null){
			//it is persistent, but it isn't in the database !?
			return true;
		}
		
		EntityManager em = null;
		try {
			em = this.getCurrentEntityManager();
		} catch (Throwable t) {
			throw new RuntimeException(t);
		}
		...
		try {
			...
			em.refresh(timerEntity, LockModeType.PESSIMISTIC_WRITE, properties);
	 ...
	 * <p>
	 * Note that, suspend does <b>not</b> cancel the {@link Timer}. Instead, it
	 * just cancels the <b>next scheduled timeout</b>. So once the {@link Timer}
	...
	/**
	 * Restores persisted timers, corresponding to this timerservice, which are
	 * eligible for any new timeouts.
	 * <p>
	 * This includes timers whose {@link TimerState} is <b>neither</b> of the
	 * following:
	 * <ul>
	...
	public void restoreTimers() {
		...
		logger.debug("Found " + restorableTimers.size()
				+ " active timers for timedObjectId: "
				+ this.invoker.getTimedObjectId());
		// now "start" each of the restorable timer. This involves, moving the
		// timer to an ACTIVE state
		// and scheduling the timer task
		for (TimerImpl activeTimer : restorableTimers) {
			this.startTimer(activeTimer);
			logger.debug("Started timer: " + activeTimer);
			// save any changes to the state (that will have happened on call to
			// startTimer)
	...
	/**
	 * Registers the timer with any active transaction so that appropriate
	 * action on the timer can be carried out on transaction lifecycle events,
	 * through the use of {@link Synchronization} callbacks.
	 * <p>
	...
	protected void registerTimerWithTx(TimerImpl timer) {
		...
		if (tx != null) {
			try {
				...
				tx.registerSynchronization(new TimerCreationTransactionSynchronization(
						timer));
	...
	/**
	 * Moves the timer to either {@link TimerState#STARTED_IN_TX} or
	 * {@link TimerState#ACTIVE} depending on whether there's any transaction
	 * active currently.
	 * <p>
	 * If there's no transaction currently active, then this method creates and
	 * schedules a timer task. Else, it just changes the state of the timer to
	 * {@link TimerState#STARTED_IN_TX} and waits for the transaction to commit,
	 * to schedule the timer task.
	 * </p>
	 * 
	 * @param timer
	 */
	...
	/**
	 * Returns true if the {@link CurrentInvocationContext} represents a
	 * lifecycle callback invocation. Else returns false.
	 * <p>
	 * This method internally relies on {@link CurrentInvocationContext#get()}
	 * to obtain the current invocation context.
	 * <ul>
	 * <li>If the context is available then it looks for the method that was
	 * invoked. The absence of a method indicates a lifecycle callback.</li>
	 * <li>If the context is <i>not</i> available, then this method returns
	 * false (i.e. it doesn't consider the current invocation as a lifecycle
	 * callback). This is for convenience, to allow the invocation of
	 * {@link javax.ejb.TimerService} methods in the absence of
	 * {@link CurrentInvocationContext}</li>
	 * </ul>
	 * 
	 * </p>
	 * 
	...
	/**
	 * Creates and schedules a {@link TimerTask} for the next timeout of the
	 * passed <code>timer</code>
	 */
	protected void scheduleTimeout(TimerImpl timer) {
		Date nextExpiration = timer.getNextExpiration();
		if (nextExpiration == null) {
			logger.info("Next expiration is null. No tasks will be scheduled for timer "
					+ timer);
			return;
		}
		...
		if (intervalDuration > 0) {
			logger.debug("Scheduling timer " + timer
					+ " at fixed rate, starting at " + delay
					+ " milli seconds from now with repeated interval="
					+ intervalDuration);
			// schedule the task
			Future<?> future = this.executor.scheduleAtFixedRate(timerTask,
					delay, intervalDuration, TimeUnit.MILLISECONDS);
			// maintain it in timerservice for future use (like cancellation)
			this.scheduledTimerFutures.put(timer.getTimerHandle(), future);
		} else {
			logger.debug("Scheduling a single action timer " + timer
					+ " starting at " + delay + " milli seconds from now");
			// schedule the task
			Future<?> future = this.executor.schedule(timerTask, delay,
	...
	/**
	 * Cancels any scheduled {@link Future} corresponding to the passed
	 * <code>timer</code>
	 * 
	 * @param timer
	 */
	...
	private TimerImpl getPersistedTimer(TimerHandleImpl timerHandle) {
		...
		Query query = em
				.createQuery("from TimerEntity t where t.id = :id and t.timedObjectId = :timedObjectId");
		...
		if (timers.size() > 1) {
			throw new EJBException(
					"More than one timer found for TimerHandle: " + timerHandle);
		}
	...
	private org.jboss.ejb3.timerservice.extension.Timer getExistingAutoTimer(
			ScheduleExpression schedule, TimerConfig timerConfig,
		...
		//
		// List<CalendarTimerEntity> autoTimers =
		// autoTimersQuery.getResultList();
		// for (CalendarTimerEntity autoTimer : autoTimers)
		// {
		// TimeoutMethod timeoutMethod = autoTimer.getTimeoutMethod();
		// if (this.doesTimeoutMethodMatch(timeoutMethod, timeoutMethodName,
		// methodParams) == false)
		// {
		// continue;
		// }
		...
		// }
		// // now onto schedule
		// ScheduleExpression autoTimerSchedule =
		// autoTimer.getScheduleExpression();
	...
	private boolean doSchedulesMatch(ScheduleExpression schedule,
			ScheduleExpression otherScheduleExpression) {
		return true;
	}

	private boolean doesTimeoutMethodMatch(TimeoutMethod timeoutMethod,
			String timeoutMethodName, String[] methodParams) {
	...
	private boolean methodParamsMatch(String[] methodParams,
			String[] otherMethodParams) {
	...
	/**
	 * Marks the transaction for rollback NOTE: This method will soon be
	 * removed, once this timer service implementation becomes "managed"
	 * 
	 */
	...
	// changed the visibility to control the transaction from the timertask
	...
	public void endTx() {
		try {
			...
			if (tx == null) {
				throw new IllegalStateException(
						"Transaction cannot be ended since no transaction is in progress");
			}
	...
	private class TimerCreationTransactionSynchronization implements
			Synchronization {
		/**
		...
		public void afterCompletion(int status) {
			if (this.timer.persistent) {
				synchronized (TimerServiceImpl.this.persistentWaitingOnTxCompletionTimers) {
					TimerServiceImpl.this.persistentWaitingOnTxCompletionTimers
							.remove(this.timer.getTimerHandle());
				}
			}
	...
	private class EntityManagerTransactionSynchronization implements
			Synchronization {

		@Override
		public void afterCompletion(int status) {
			EntityManager em = TimerServiceImpl.this.transactionScopedEntityManager
					.get();
			...
			if (em != null) {
				try {
					...
					logger.debug(
							"Ignoring exception during entity manager close: ",
							e);
				}
			}
	...
	private EntityManager getCurrentEntityManager() throws Exception {
		...
		if (tx == null) {
			throw new IllegalStateException(
					"No transaction in progress. Cannot create an entity manager");
		}
